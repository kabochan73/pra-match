// 企業のメッセージ画面。求職者側と同じ構造でsender_typeの判定だけ異なる。
...
export default async function CompanyMessageThreadPage({ params }: Props) {
  const { matchingId } = await params
  const token = await getToken()

  const [matchingsList, messages] = await Promise.all([
    apiGet<Matching[]>('/company/matchings', token).catch(() => [] as Matching[]),
    apiGet<Message[]>(`/matchings/${matchingId}/messages`, token).catch(() => [] as Message[]),
  ])
  const matching = matchingsList.find((m) => m.id === Number(matchingId)) ?? null
  ...
  return (
    <div className="flex flex-col h-screen">
      ...
      <header className="bg-white border-b border-gray-100 px-4 py-4 flex items-center gap-3 shrink-0">
        <Link href="/company/messages" className="text-gray-400 hover:text-gray-600">
          <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15 19l-7-7 7-7" />
          </svg>
        </Link>
        <div className="min-w-0">
          <p className="font-semibold text-gray-900 truncate">{matching.user?.name}</p>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
        ...
        {messages.map((msg) => {
          const isMine = msg.sender_type === 'company'
          return (
